  ...
  var QueryGenerator = {
    ...
    */
    ...
    selectQuery: function(tableName, options, model) {
      ...
      if (subQuery) {
        subQueryItems.push('SELECT ' + (options.distinct===true ? 'DISTINCT ' : '') + subQueryAttributes.join(', ') + ' FROM ' + options.table);
        if (mainTableAs) {
      ...
      // Else do it the reguar way
        ...
        mainQueryItems.push('SELECT ' + (options.distinct===true ? 'DISTINCT ' : '') + mainAttributes.join(', ') + ' FROM ' + options.table);
        if (mainTableAs) {
